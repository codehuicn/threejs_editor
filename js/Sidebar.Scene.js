...
Sidebar.Scene = function ( editor ) {
	...
	fogTypeRow.add( new UI.Text( '烟雾效果' ).setWidth( '90px' ) );
	...
	var fogNear = new UI.Number( 0.1 ).setWidth( '30px' ).setRange( 0, Infinity ).onChange( onFogChanged );
	fogPropertiesRow.add( fogNear );

	// fog far

	var fogFar = new UI.Number( 50 ).setWidth( '50px' ).setRange( 0, Infinity ).onChange( onFogChanged );
	fogPropertiesRow.add( fogFar );

	// fog density

	var fogDensity = new UI.Number( 0.05 ).setWidth( '50px' ).setRange( 0, 0.1 ).setPrecision( 3 ).onChange( onFogChanged );
	...
	function refreshUI() {
		...
		options.push( buildOption( scene, true ) );
